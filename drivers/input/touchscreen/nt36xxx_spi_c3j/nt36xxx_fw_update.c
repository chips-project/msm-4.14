...
#define pr_fmt(fmt)	"nt36xxx_fw_upd: %s: " fmt, __func__
...
static int32_t update_firmware_request(char *filename)
{
	...
	while (1) {
		...
		if (nvt_get_fw_need_write_size(fw_entry)) {
			pr_err("get fw need to write size fail!\n");
			ret = -EINVAL;
			goto invalid;
		}
		...
		if (ret) {
			pr_err("bin header parser failed\n");
			goto invalid;
		} else {
			break;
		}
...
static int32_t nvt_check_fw_checksum(void)
{
	...
	if (ret) {
		pr_err("Read fw checksum failed\n");
		return ret;
	}
	...
	for (list = 0; list < partition; list++) {
		...
		if (bin_map[list].crc != fw_checksum) {
			pr_err("[%d] BIN_checksum=0x%08X, FW_checksum=0x%08X\n",
...
static int32_t nvt_download_firmware(void)
{
	...
	while (1) {
		...
		if (ret) {
			pr_err("firmware checksum not match, retry=%d\n", retry);
			goto fail;
		} else {
			break;
		}
...
int32_t nvt_update_firmware(char *firmware_name)
{
	...
	if (ret) {
		pr_err("Download Firmware failed. (%d)\n", ret);
		goto download_fail;
	}
	...
	ret = nvt_get_fw_info();
	if (ret)
		pr_err("nvt_get_fw_info failed. (%d)\n", ret);
